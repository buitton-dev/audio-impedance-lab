...
ギターとオーディオ配線の回路定数を動かし、回路図・周波数特性・Smith chartを同時に確認できる、ブラウザだけで動く教材です。
...
接続モデルの信号源側から見たインピーダンス `Zin` を、選択した基準インピーダンス `Z0` で正規化し、

`Γ = (Zin - Z0) / (Zin + Z0)`

として表示します。オーディオケーブルを分布定数線路として扱う意図ではなく、複素インピーダンスとRFで使う表示法のつながりを学ぶための表示です。
